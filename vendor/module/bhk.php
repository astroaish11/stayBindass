...
if(isset($_POST['submit'])){
   ...
   /***********************************INSERT & UPDATE*****************************************/
    ...
    if($eid == ''){
        ...
        mysqli_query($sql, "INSERT INTO `bhk_master` (`bhk`, `created_by`, `created_date`) VALUES ('$bhk', '$userid', '$cdate')");

        echo '<script type="text/javascript">window.location.href="bhk_master.php?s=1"</script>';
        
    }else {
        mysqli_query($sql, "UPDATE `bhk_master` SET `bhk`='$bhk', `updated_by`='$userid', `updated_date`='$cdate' where id = '$eid'");
...
if(isset($_GET['did'])) {
    ...
    mysqli_query($sql, "UPDATE `bhk_master` SET `deleted` = 1, `updated_date` = '$cdate', `updated_by` = '$eid' where `id` = '$did'");
