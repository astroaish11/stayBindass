...
if (isset($_POST['submit'])) {
    ...
    if ($eid == '') {
       ...
       $propertyQuery =  "INSERT INTO `addproperty` (`title`,`actualtitle`,`slug`,`state`,`city`,`address`,`h_type`,`r_type`,`minguest`, `maxguest`,`service_charge`, `nights`, `pool`, `destination`, `propcat`, `amenities`,`villaQuantity`, `property_by`,`overview`,`seo_title`,`seo_keyword`,`seo_desc`,`location_link`,`video_link`,`rules`,`security`,`policies`,`shortdesc`,`uploadimage`,`latitude`,`longitude`,`created_date`,`created_by`) VALUES ('$title','$actualtitle','$slug','$state','$city','$address','$home_type','$room_type','$minguest','$maxguest','$serviceCharges', '$nights', '$pool', '$destination', '$propcat', '$amenities','$villaQuantity','$property','$overview','$seo_title','$seo_keyword','$seo_desc','$location_link','$video_link','$rules','$security','$policies','$shortdesc','$uploadimage','$latitude','$longitude','$cdate','1')";
...
if (isset($_GET['did'])) {
    ...
    echo '<script type="text/javascript">window.location.href="manage_listing.php?d=1"</script>';
...
$villaID = $_SESSION[SESSION]['loginuserid'];
...
$query = mysqli_query($sql, "SELECT a.*, r.room, v.name as vendorname, d.title as titlename, s.name, c.city as cityname 
                        ...
                        left Join `awt_states` as s on s.id = a.state 
                        left Join `city_master` as c on c.id = a.city 
                        where a.deleted != 1 and a.adminApproval=1 and a.property_by= $villaID");
