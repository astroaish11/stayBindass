...
if(isset($_POST['submit1'])) {
    ...
    // if($eid == ''){
    //     mysqli_query($sql,"INSERT INTO `awt_vgallery` (`type`,`v_id`, `videoTitle`, `link`,`video`,`created_date`,`created_by`) VALUES ('2','$vid', '$title','$link','$image','$cdate','1')");
        
    //     echo '<script type="text/javascript">window.location.href="video.php?s=1"</script>';
    // }
    // else{
    //     mysqli_query($sql,"UPDATE `awt_vgallery` SET `type`='2',`v_id`='$vid', `videoTitle`='$title',`link`='$link',`video`='$image',`updated_by`='1',`updated_date`='$cdate' WHERE id = '$eid'");
...
if(isset($_GET['eid'])) {
    ...
    $selectdata = mysqli_query($sql, "SELECT * FROM `awt_vgallery` WHERE `id` = '$eid'");
...
$query = mysqli_query($sql, "SELECT * FROM `awt_vgallery` WHERE deleted != '1'");
