...
if(isset($_GET['datesubmit'])){
    ...
    for($i=0; $i<=$days; $i++){
        ...
        if($count == 0){
            ...
            if($day == "monday"){
                $price = $getdata_day->monday;
                $guest_price = $getdata_day->guestmonday;
            }
            elseif($day == "tuesday"){
                $price = $getdata_day->tuesday;
                $guest_price = $getdata_day->guesttuesday;
            }
            elseif($day == "wednesday"){
                $price = $getdata_day->wednesday;
                $guest_price = $getdata_day->guestwednesday;
            }
            elseif($day == "thursday"){
                $price = $getdata_day->thursday;
                $guest_price = $getdata_day->guestthursday;
            }
            elseif($day == "friday"){
                $price = $getdata_day->friday;
                $guest_price = $getdata_day->guestfriday;
            }
            elseif($day == "saturday"){
                $price = $getdata_day->saturday;
                $guest_price = $getdata_day->guestsaturday;
            }
            else{
                $price = $getdata_day->sunday;
                $guest_price = $getdata_day->guestsunday;
            }
...
if(isset($_POST['priceupdate'])){
    ...
    for($i=1;$i<=$row_count;$i++){
        ...
        $p_price = $_POST['prop_price'.$i];
        $g_price = $_POST['g_price'.$i];
