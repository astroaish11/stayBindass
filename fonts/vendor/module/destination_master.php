...
if(isset($_POST['submit'])) {
    ...
    if($eid == ''){
        mysqli_query($sql,"INSERT INTO `destination_master` (`title`,`slug`,`logo`,`link`,`openin`,`created_date`,`created_by`) VALUES ('$title', '$slug', '$image','$nlink','$target','$cdate','1')");
        
        echo '<script type="text/javascript">window.location.href="destination_master.php?s=1"</script>';
    }
    else{
        mysqli_query($sql,"UPDATE `destination_master` SET `title`='$title', `slug`='$slug',`link`='$nlink',`logo`='$image',`openin`='$target',`updated_by`='1',`updated_date`='$cdate' WHERE id = '$eid'");
        echo '<script type="text/javascript">window.location.href="destination_master.php?u=1"</script>';
    }

    $query_json = mysqli_query($sql, "SELECT * FROM `destination_master` WHERE `deleted` != '1'");
