...
if(isset($_POST['submit'])) {
    ...
    if($eid == ''){
        mysqli_query($sql,"INSERT INTO `awt_vgallery` (`type`,`v_id`, `title`,`logo`,`created_date`,`created_by`) VALUES ('1','$vid', '$title','$image','$cdate','1')");
        
        echo '<script type="text/javascript">window.location.href="gallery.php?vid='.$vid.'&s=1"</script>';
    }
    else{
        mysqli_query($sql,"UPDATE `awt_vgallery` SET  `type`='1',`v_id`='$vid', `title`='$title',`logo`='$image',`updated_by`='1',`updated_date`='$cdate' WHERE id = '$eid'");
