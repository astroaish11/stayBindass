...
if(isset($_POST['blockdate_update'])){
    ...
    for($i=1;$i<=$row_count;$i++){
      ...
      //  $quantity1 = $_POST['quantity1' . $i];
          if($blocked_date == 1){
             // mysqli_query($sql,"UPDATE `awt_blockDates` set `blockStatus`= 1, `block_quantity`='$quantity' where `select_date`='$select_date' and `property_id`='$hidden_propid'");
         ...
         if($blocked_date == 0){
            // mysqli_query($sql,"UPDATE `awt_blockDates` set `blockStatus`= 0, `block_quantity`='$quantity' where `select_date`='$select_date' and `property_id`='$hidden_propid'");
             echo "UPDATE `awt_blockDates` set `blockStatus`= 1, `block_quantity`='$quantity' where `select_date`='$select_date' and `property_id`='$hidden_propid'";
         }
